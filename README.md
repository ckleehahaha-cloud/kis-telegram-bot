...
| `/vol` | `/volatility` | KOSPI 심리 변동 비율 — 최근 6개월 + 텍스트 요약 ([상세](#volatility)) |
| `/gl` | `/global` | 글로벌 시가총액 Top 30 텍스트 (~15초 소요) |
...
차트 표시 범위는 **최근 6개월**입니다.

차트 전송 후 텍스트 요약도 함께 출력됩니다.
...
|----|------|
| MCap | 시가총액 (조 원, T KRW) |
| F.NI | Forward 순이익 — 컨센서스 추정치 (조 원) |
| FPER | Forward PER — 현재가 / Forward EPS |
| FY/Mo | Forward EPS 기준 회계연도 / 결산월 (`yy/mm`) |

- 해외 종목: yfinance 시총 + yahooquery/yfinance Forward EPS 기반 순이익 추정
- 한국 종목(Samsung Elec, SK Hynix): 네이버 금융 실시간 시총 + 컨센서스 순이익
- FY/Mo: 비KS는 `nextFiscalYearEnd` 타임스탬프 기반, KS는 3/31 기준 당해/전년 12월
- 환율: yfinance 병렬 조회. 실제 사용된 통화만 푸터에 표시. 실패 시 내장 기본값 사용
- 티커 캐시: 60분간 프로세스 캐시 (재조회 불필요)
- 스크래핑 실패 시 하드코딩 30개 티커로 자동 전환
- 출력은 모노스페이스 코드 블록으로 열 정렬 (기업명 최대 15자, 영문)
- 병렬 처리(ThreadPoolExecutor + yahooquery 배치)로 응답 시간 단축
...
- `/gl` · `/global` 은 데이터 수집에 약 15초 소요됩니다. F.NI/FPER/FY·Mo 는 Forward(추정치)이며 실적값이 아닙니다.
...
- **로그**: 봇 시작 시 `bot.log` 파일에 자동 기록됩니다. 5MB 초과 시 `bot.log.1~3`으로 순환 저장되며 최대 20MB를 초과하지 않습니다.
- **data/ 파일 자동 정리**: 봇 시작 시 및 매일 08:30에 `data/program_*.json` 중 2일 이전 파일을 자동 삭제합니다.
