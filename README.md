...
투자자 수급, 프로그램 매매, 손익계산서, 재무비율, 밸류에이션, 현금흐름표를 차트 이미지로 전송합니다.
...
| `/val 삼성전자` | `/valuation` | 밸류에이션 차트 (EPS/BPS/DPS/PER/PBR, 연간) |
| `/cf 삼성전자` | `/cashflow` | 현금흐름표 차트 (영업/투자/재무CF + FCF, 연간+분기) |
...
| 데이터 | 소스 |
|--------|------|
| 수급, 프로그램 매매, 현재가 | KIS OpenAPI |
| 손익계산서, 재무비율, 밸류에이션 | KIS OpenAPI |
| 주당배당금 (DPS) | DART OpenAPI (`alotMatter`) |
| 현금흐름표 | DART OpenAPI (`fnlttSinglAcntAll`) |
...
- `/cf`, `/val` 명령어는 DART API Key가 없으면 동작하지 않습니다.
