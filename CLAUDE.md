...
├── globaltop30.py   # 글로벌 시총 참고용 스크립트 (봇에서 미사용, 삭제 금지)
...
| `/vol` | `/volatility` | KOSPI 심리 변동 비율 — RobustSTL 잔차 비율 최근 3개월 (수십 초 소요) |
...
- `_send_global` — `/global` 핸들러. `global_api.get_global_data()` → `(df, usd_krw)` 언패킹. `asyncio.to_thread`로 실행. 헤더 열: `시총(조)` / `F순이익` / `FPER` (F = Forward 추정치).
...
- `_send_volatility` — `/vol` 핸들러. yfinance로 KOSPI 3년치 다운로드 → `RobustSTL` 분해 → 잔차 비율 91일. CPU 집약 작업은 `asyncio.to_thread`로 실행.
...
- `_ljust_disp(s, width)` — CJK 2칸 너비 기반 왼쪽 정렬 패딩. `/global` 테이블 출력에 사용.
...
| `chart_volatility(dates, resid_ratio)` | KOSPI 잔차 비율 막대 (최근 3개월). 양수=빨강, 음수=파랑, ±1σ 기준선 표시 |
...
- `/vol` — yfinance로 KOSPI 3년치 다운로드 후 RobustSTL 분해. `scipy.optimize.linprog`(HiGHS) LP 포함으로 **수십 초 소요**. `asyncio.to_thread`로 이벤트 루프 블로킹 방지.
  ...
  - 표시 범위: 마지막 날짜 기준 91일(약 3개월) 슬라이싱.
  - ±1σ 기준선으로 과매수/과매도 판단 참고 가능.
- `/global` — companiesmarketcap.com 파싱 실패 시 `_FALLBACK_TICKERS`(하드코딩 30개)로 자동 전환.
  티커 리스트는 60분간 프로세스 캐시. 한국 종목 시총은 네이버 금융(`res.text`, UTF-8) 파싱.
  `_market_sum` 태그 형식: `'874조\n4,858'` → `874 + 4858/10000 = 874.4858` 조원.
  출력 열: `시총(조)` / `F순이익` / `FPER` — F 접두사는 Forward(컨센서스 추정치) 표시. 하단 주석 `F순이익/FPER = Forward (컨센서스 추정치)` 자동 출력.
...
- 수정 금지 파일: `config.py`, `collector.py`, `.stock_list.json`, `globaltop30.py`
...
- 모든 텍스트 `send_message` / `reply_text`는 `parse_mode` 없는 일반 텍스트. 백틱(`` ` ``) 코드 포맷 사용 금지. 차트 `send_photo` 캡션과 ⏳/❌ 상태 메시지만 `parse_mode="Markdown"` 허용.
