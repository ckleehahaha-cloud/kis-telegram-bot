...
- `_send_global` — `/global` 핸들러. `global_api.get_global_data()` → `(df, usd_krw)` 언패킹. `asyncio.to_thread`로 실행.
...
- `cmd_cstocks` — `/cs` 핸들러. `config.COLLECTOR_STOCKS` 런타임 수정. 서브커맨드: (없음)=목록, `add 코드`, `del 코드`.
