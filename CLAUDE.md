...
- `_send_global` — `/gl` · `/global` 핸들러. `global_api.get_global_data()` → `(df, usd_krw)` 언패킹. `asyncio.to_thread`로 실행. 출력: 모노스페이스 코드 블록(`parse_mode="Markdown"`). 헤더 열: `#` / `Name`(20자) / `MCap(T)` / `F.NI(T)` / `FPER`. F = Forward(컨센서스 추정치).
...
- `/gl` · `/global` — 단축 `/gl` 추가. companiesmarketcap.com 파싱 실패 시 `_FALLBACK_TICKERS`(하드코딩 30개)로 자동 전환.
  ...
  출력: 모노스페이스 코드 블록. 열: `#` / `Name`(20자) / `MCap(T)` / `F.NI(T)` / `FPER`.
  하단 주석 `F.NI/FPER = Forward (컨센서스 추정치)` 자동 출력.
