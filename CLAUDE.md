...
├── RobustSTL.py    # RobustSTL 시계열 분해 클래스 (/volatility에서 사용)
...
| `/sh 종목명` | `/short` | 공매도·대차잔고 추이 차트 (최근 3개월) |
...
| `/volatility` | `/volatility` | KOSPI 심리 변동 비율 — RobustSTL 잔차 비율 최근 60거래일 (yfinance, 수십 초 소요) |
...
| `/fa 종목명` | `/financeall` | 재무 전체 — fin·r·val·cf·sum·div·pr·sh 순서로 실행 |
...
| `get_short_history(code, days=90)` | FHPST04010000 + FHPST04020000 | 주/원 | 공매도 일별 추이 + 대차잔고 병합. fallback TR: FHPST01010400 / FHPST01020400 |
...
| `chart_valuation(annual, name)` | 밸류에이션 5패널 연간 막대: EPS/BPS/PER(주가순익비율)/POR(주가영익비율)/PBR(주가순자산비율) |
...
| `chart_volatility(dates, resid_ratio)` | KOSPI 잔차 비율 막대 (최근 60거래일). 양수=빨강, 음수=파랑, ±1σ 기준선 표시 |
| `chart_short(data, name)` | 공매도·대차잔고 3행 공유x축. Row0=종가, Row1=공매도비율+5일MA, Row2=대차잔고fill |
...
- `_send_finance_all` — `/fa` 핸들러. `_send_finance` → `_send_ratio` → `_send_valuation` → `_send_cashflow` → `_send_summary` → `_send_dividend` → `_send_pricerange` → `_send_short` 순서로 순차 실행.
- `_send_pricerange` — `/pr` 핸들러. `get_price_range_history` + `get_dividend_per_share`(DPS 보완) → `chart_price_range` + raw data 텍스트.
- `_send_short` — `/sh` `/short` 핸들러. `get_short_history` → `chart_short` + 최신 5행 텍스트 요약.
- `_send_dupont` — `/du` `/dupont` 핸들러. `get_dupont_data` → `chart_dupont` + raw data 텍스트.
- `_send_volatility` — `/volatility` 핸들러. yfinance로 KOSPI 3년치 다운로드 → `RobustSTL` 분해 → 잔차 비율 60일 → `chart_volatility`. CPU 집약 작업은 `asyncio.to_thread`로 실행.
- `_start_collector()` / `_stop_collector()` — 봇 내부에서 수집기 스레드 제어.
- `ALLOWED_USER_IDS`가 설정된 경우 모든 명령어 앞에서 접근 제어.

### `RobustSTL.py`

논문(RobustSTL, 2018) 알고리즘 구현. `/volatility` 명령어에서 단독 사용.

- `RobustSTL(y, period, reg1, reg2, K, H)` — 시계열 분해 클래스
  - `bilateral_filter` — Step 1: 양방향 필터링으로 노이즈 제거
  - `extract_trend` — Step 2: L1-norm 최적화(cvxpy)로 강건 추세 추출
  - `extract_seasonality` — Step 3: 비국소 계절성 필터링
  - `fit(iterations=1)` — 전체 분해 실행. `.trend` / `.seasonal` / `.resid` 반환
- 의존성: `numpy`, `cvxpy`, `scipy.sparse`
...
- `get_short_history` — 공매도 TR(`FHPST04010000`)과 대차잔고 TR(`FHPST04020000`) 순차 호출(sleep 0.3).
  404 응답 시 `FHPST01010400` / `FHPST01020400` fallback. 어느 TR이 응답했는지 INFO 로그.
  short_ratio = smsl_qty / acml_vol × 100. 데이터 없는 쪽 필드는 None 반환.
...
- `/volatility` — yfinance로 KOSPI 3년치 다운로드 후 RobustSTL 분해. cvxpy L1 최적화 포함으로 **수십 초 소요**. `asyncio.to_thread`로 이벤트 루프 블로킹 방지.
