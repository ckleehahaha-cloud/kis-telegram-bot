...
| `/val 종목명` | `/valuation` | 밸류에이션 차트 (EPS/BPS/DPS/PER/PBR, 연간) |
...
| `get_price_range_history(code)` | — | 원 | `get_valuation_ratio`+`get_price_history("Y")` 병합. 연간 EPS/BPS/주가Min·Max. dps=None(bot layer에서 채움) |
...
| `chart_price_range(data, name)` | 주가범위 2행 패널. Row1=EPS/DPS 그룹막대, Row2=주가 Min·Max 밴드+중간선 |
